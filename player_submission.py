...
class CustomPlayer:
    ...
    def utility_custom(self, game, maximizing_player=True):
    ...
    def minimax(self, game, time_left, depth=3, maximizing_player=True):
        ...
        # raise NotImplementedError
        best_move = (-1, -1)
        best_score = self.utility(game, maximizing_player)
    ...
    def alphabeta(self, game, time_left, depth=3, alpha=float("-inf"), beta=float("inf"),
        ...
        if maximizing_player:
            ...
            for move in legal_moves:
                ...
                _, score = self.alphabeta(next_state, time_left, depth - 1, alpha, beta, False)
                if score > best_score:
                    best_score = score
                    best_move = move
                if best_score >= beta:
            ...
            for move in legal_moves:
                next_state = game.forecast_move(move)
                _, score = self.alphabeta(next_state, time_left, depth - 1, alpha, beta, True)
                if score < best_score:
                    best_score = score
                    best_move = move
                if alpha >= best_score:
