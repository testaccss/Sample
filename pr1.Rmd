...
n_na_gross = length(df2$Gross[!is.na(df2$Gross)])
cat('The number of gross value present:', n_na_gross)
df2$Released_Year = as.numeric(substr(df2$Released,1,4))
df2_rel_year = subset(df2, (df2$Released_Year == df2$Year))
df2_rel_year_subset = length(df2$Gross[!is.na(df2$Gross)])
df2_date_year = subset(df2, (df2$Date == df2$Year))
cat('\nThe number of release and year matching:', nrow(df2_rel_year))
